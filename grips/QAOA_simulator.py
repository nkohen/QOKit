...
def get_overlap_min(
    ...
    simulator, result = get_simulator_and_result(N, terms, gamma, beta, sim, result, simulator_name=simulator_name)
    return simulator.get_overlap(result, preserve_state=True, optimization_type="min")
...
def inverse_objective_function(
    ...
    def inverse_objective(*args) -> float:
        ...
        if overlaps is not None:
            overlaps.append((current_time, get_overlap_max(N, ising_model, gamma, beta, simulator, result, simulator_name=simulator_name)))
...
def objective_function(
    ...
    def objective(*args) -> float:
        ...
        if overlaps is not None:
            overlaps.append((current_time, get_overlap_min(N, ising_model, gamma, beta, simulator, result, simulator_name=simulator_name)))
...
def QAOA_run_max(
    ...
    return {
        ...
        "overlap": get_overlap_max(N, ising_model, gamma, beta, simulator_name=simulator_name),
...
def QAOA_run_min(
    ...
    start_time = time.time()
    result = scipy.optimize.minimize(
        objective_function(ising_model, N, p, mixer, expectations, overlaps, simulator_name=simulator_name),
        init_freq,
        args=(),
        method=optimizer_method,
        options=optimizer_options,
    ...
    return {
        ...
        "state": get_state(N, ising_model, gamma, beta, simulator_name=simulator_name),
        "expectation": get_expectation(N, ising_model, gamma, beta, simulator_name=simulator_name),
        "overlap": get_overlap_min(N, ising_model, gamma, beta, simulator_name=simulator_name),
        "runtime": end_time - start_time,  # measured in seconds
        "num_QAOA_calls": result.nfev,
        "classical_opt_success": result.success,
        "scipy_opt_message": result.message,
